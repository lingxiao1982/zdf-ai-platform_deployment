...
const PROJECT_NAME = 'zdf-ai-platform';
const FRONTEND_DIR = path.join(__dirname, PROJECT_NAME);

console.log(`🚀 开始生成 [${PROJECT_NAME}] 全栈项目结构...`);
...
const packageJson = `{
  ...
  "scripts": {
    "dev": "vite",
    "dev:full": "concurrently -k -n web,api -c cyan,magenta \\"vite\\" \\"npm run dev --prefix backend\\"",
    "build": "vite build",
...
fs.writeFileSync(path.join(FRONTEND_DIR, 'package.json'), packageJson);
...
fs.writeFileSync(path.join(FRONTEND_DIR, 'vite.config.js'), viteConfig);
...
fs.writeFileSync(path.join(FRONTEND_DIR, 'tailwind.config.js'), tailwindConfig);
...
fs.writeFileSync(path.join(FRONTEND_DIR, 'postcss.config.js'), postcssConfig);
...
@layer base {
  body { @apply antialiased text-gray-900 bg-gray-50; }
...
const appJsx = `import React, { useState, useEffect, useRef } from 'react';
import { 
          ...
          {subTab === 'apikeys' && (
            <div className="space-y-4 max-w-4xl">
              ...
              {VENDORS.map(v => {
                ...
                return (
                  <div key={v.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between shadow-sm">
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <button onClick={() => handleTestKey(v.id)} disabled={kSt.status==='testing'||!kSt.value} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm rounded-lg transition-colors">测试</button>
...
// 🔧 从仓库模板复制 Node 后端（若存在）
// ==========================================
const backendSrcDir = path.join(__dirname, PROJECT_NAME, 'backend');
const backendDestDir = path.join(FRONTEND_DIR, 'backend');
if (
  fs.existsSync(path.join(backendSrcDir, 'server.js')) &&
  path.resolve(backendSrcDir) !== path.resolve(backendDestDir)
) {
  fs.cpSync(backendSrcDir, backendDestDir, { recursive: true });
}

// ==========================================
// 🚀 3. 安装依赖并给出提示
// ==========================================
console.log('✅ 项目结构创建完成！正在自动安装依赖 (请稍候...)');
...
try {
  ...
  console.log('  cd backend && npm install && cd ..   # 首次安装后端依赖');
  console.log('  npm run dev:full  # 同时启动 Vite(8080) + API(3000)，可测多模型调度');
  console.log('  npm run dev       # 仅前端（需自行另开终端启动 backend）');
  console.log('  npm run build     # 编译前端；生产可 npm start（由 backend 托管 dist）');
  console.log('=============================================');
} catch (error) {
  console.error('❌ 依赖安装失败，请手动进入目录执行 npm install');
}